...
$TargetTestProject = "OpenJpegDotNet.UWP.Tests.csproj"
...
function Update-Version([string]$TestProjectDir, [string]$Version)
{
   ...
   $nodes = $xml.SelectNodes("//msb:ItemGroup/msb:PackageReference[contains(./@Include , 'OpenJpegDotNet.UWP')]", $ns)
...
function AppCert([string]$Architecture, [string]$OutputDir)
{
   ...
   $appx  = Join-Path $OutputDir OpenJpegDotNet.UWP.Tests |
            Join-Path -ChildPath OpenJpegDotNet.UWP.Tests_1.0.0.0_${Architecture}.appx
   $report   = Join-Path $OutputDir OpenJpegDotNet.UWP.Tests |
...
function main([string]$Version, [string]$Thumbprint, [string]$CertificateKeyFile, [string]$NotCountAsFailTests)
{
   ...
   {
      $testDirectory =  Join-Path $OpenJpegDotNetRoot test |
                        Join-Path -ChildPath OpenJpegDotNet.UWP.Tests
